...
std::string decode(std::string encryptedStr){
	double real_Frequencies = {8.2, 1.5, 2.8, 4.3, 13,2.2, 2, 6.1, 7, 0.15,0.77, 4, 2.4, 6.7, 7.5,1.9, 0.095, 6, 6.3, 9.1,2.8, 0.98, 2.4, 0.15, 2,0.074};
	char alphabet [26] = {'a','b','c','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t','u','v','w','x','y','z'};
	std::string sentence[26]; //storing frequencies of encrypted sentence here
	double distance[26]; 
	
	//adding the frequency to vector: sentence using above function charfrequencyEncryption
	for(int i=0;i<26;i++){
		sentence+=charfrequencyEncryption(encryptedStr, alphabet[i]);
	}
	...
	*/
	...
	/*int charDistance = 0;
	for(int i=0; i<26; i++){
		powEqnPart+=pow((real_Frequencies[i]-sentence[i]),2);
	}
	distanceCalculated += sqrt(powEqnPart);
		//adding it to the distance vector (created above)
		distance += charDistance[i];
	*/
	...
	return decryptCaesar(encryptedStr, num_shift);
}
